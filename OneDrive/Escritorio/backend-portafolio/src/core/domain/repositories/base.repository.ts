...
export class BaseRepository implements BaseOperations {
  ...
  async getPortafolioByUser(userid: string | number) {
    return await this.model.find({ creadoPor: userid, status: true });
  }

  async getArchivoByUser(portafolioid: string | number) {
    return await this.model.find({ portafolio: portafolioid, status: true });
  }
